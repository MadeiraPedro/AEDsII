    ...
    public static void main(String[] args) throws IOException {
		...
		for(int i = 0; i < nomes.size(); i++){
			...
			for(int j = 0; j < nomes.size(); j++){
				...
				if(nomes.get(i).equals(musica[posicoes[j]].getName())){
					temp = posicoes[i];
					...
					j = nomes.size(); //parando execucao do for(j)
				}
		...
		BufferedWriter log = new BufferedWriter(new FileWriter("matricula_selecaoParcial.txt"));
